...
function ContinentsMenu() {
    ...
    const continents = [
        ...
        { name: "North Africa", icon: GlobalIcon, isActive: false, query: "north-africa" },
        { name: "Middle East", icon: GlobalIcon, isActive: false, query: "middle-east" },
        { name: "North Africa", icon: GlobalIcon, isActive: false, query: "north-africa" },
        { name: "North Africa", icon: GlobalIcon, isActive: false, query: "north-africa" },
        { name: "North Africa", icon: GlobalIcon, isActive: false, query: "north-africa" }
    ];
    ...
    return (
        <section className="w-full flex items-center flex-wrap mt-5 ps-6 py-4">
            <div className={"w-full items-center justify-start text-[12px] gap-2 flex flex-nowrap overflow-x-auto"}>
                {continents.map((continent, index) => (
                    <div
                        key={index}
                        onClick={() => handleFilterClick(continent.query)}
                        className={`cursor-pointer flex border min-w-fit overflow-hidden flex-nowrap text-nowrap ${continent.isActive ? 'bg-blue-50 border-blue-200 text-blue-400' : 'text-gray-500'} gap-1 p-2 rounded-full`}>
