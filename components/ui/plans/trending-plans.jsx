            ...
            <section className='w-full flex flex-nowrap overflow-x-auto gap-4 items-center pb-5 ps-6 mt-[10px]'>
                {trendingItems.map((item, index) => (
                    <figure key={index}
                        ...
                        <Image src={item.image} width={135} height={87} className="rounded-xl" alt={`trend-${index + 1}`} />
                        <figcaption className={"w-full h-[58px] p-2 flex flex-wrap content-between"}>
                            <strong className="w-full flex h-2/3 text-[10px] line-clamp-2">{item.title}</strong>
                            <div className={"w-full flex mt-1 h-1/3 items-center justify-between"}>
                                <div className="w-1/2 text-[8px] gap-1 text-gray-500 flex items-center justify-start">
                                    <Image src={PersonIcon} width={8} height={8} alt={'person-icon'} />
                                    {item.personCount}
                                </div>
                                <div className="w-1/2 text-[8px] gap-1 text-gray-500 flex items-center justify-end">
                                    <Image src={LocationIcon} width={8} height={8} alt={'location-icon'} />
                                    {item.location}
                                </div>
                            </div>
                        </figcaption>
