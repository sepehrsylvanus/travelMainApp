...
function NavigationBottom() {
    ...
    return (<nav className="w-full md:w-[375px] left-1/2 px-6 -translate-x-1/2  flex flex-wrap p-2 fixed bottom-0">
        <div className="w-full flex items-center justify-between">
            <div
                className="w-4/5 bg-white h-[54px] shadow rounded-2xl flex flex-wrap items-center justify-between p-2">
                <Link href={"/"} passHref prefetch
                      className="w-1/3 flex  items-center justify-center hover:bg-gray-50 duration-200 rounded-xl p-2">
                    <Image src={HomeIcon} alt="home-icon"/>
                </Link>
                <Link href={`/${locale}/feed`} passHref prefetch
                      className="w-1/3 flex items-center  justify-center hover:bg-gray-50 duration-200 rounded-xl p-2">
                    <Image src={WindowIcon} alt="home-icon"/>
                </Link>
                <Link href={`/${locale}/nearby-people`} passHref
                      prefetch
                      className="w-1/3 flex  items-center justify-center hover:bg-gray-50 duration-200 rounded-xl p-2">
                    <Image src={LocIcon} alt="home-icon"/>
                </Link>
            </div>
            ...
            <div className="w-1/5 flex items-center justify-end">
                <button
                    ...
                    className="flex items-center bg-blue-500 w-[54px] h-[54px] rounded-2xl  justify-center active:scale-95 duration-150">
                        ...
                        <span
                            className={`text-2xl rounded-full text-white w-7 h-7 duration-300 flex items-center justify-center border ${openNav ? 'rotate-45' : 'rotate-0'}`}>
                                +
                        </span>
                </button>
            </div>
...
const DialogNavigation = ({open, locale}) => {
    ...
    return (
        <div
            className={`w-[327px] min-h-[320px] shadow-xl absolute p-2 left-1/2 text-sm -translate-x-1/2 ${open ? "bottom-[80px] opacity-100" : 'bottom-[-380px] opacity-0'} rounded-2xl duration-300 z-40 bg-white flex flex-wrap`}>
            <Link href={`/${locale}/tips-advice`} className='w-full flex  h-1/4 border-b px-3'>
            ...
            <Link href={`/${locale}/plan/create`} className='w-full flex  h-1/4 border-b px-3'>
            ...
            <Link href={`/${locale}/nearby-people/create`} className='w-full flex  h-1/4 border-b px-3'>
            ...
            <Link href={`/${locale}/tips-advice`} scroll={false} className='w-full flex  h-1/4 px-3'>
